...
[ "$INTERFACES" ] &&
 ANTENNEN=$(/usr/sbin/ubnt-discover -i "$INTERFACES" | awk '$4 ~ "[NL]B[25]|[PNB][25][BLN]|[PRNL][25]C|AG5|LM5"' | grep $LANSEGM) ||
 ANTENNEN=$(/usr/sbin/ubnt-discover                  | awk '$4 ~ "[NL]B[25]|[PNB][25][BLN]|[PRNL][25]C|AG5|LM5"' | grep $LANSEGM)
echo "${ANTENNEN[@]}" >>$LOG
