...
[ "$INTERFACES" ] &&
 ANTENNEN=$(/usr/sbin/ubnt-discover -i "$INTERFACES" | awk '$4 ~ "[NL]B[25]|[PNB][B25][BLNE]|[PRNL][25]C|AG5|LM5|LAP"' | grep $LANSEGM) ||
 ANTENNEN=$(/usr/sbin/ubnt-discover                  | awk '$4 ~ "[NL]B[25]|[PNB][B25][BLNE]|[PRNL][25]C|AG5|LM5|LAP"' | grep $LANSEGM)
echo "${ANTENNEN[@]}" >>$LOG
